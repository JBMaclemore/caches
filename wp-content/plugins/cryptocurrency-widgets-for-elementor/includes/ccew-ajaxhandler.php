...
{
    if (isset($_REQUEST['nonce']) && !wp_verify_nonce(sanitize_text_field($_REQUEST['nonce']), 'ccew-create-widget')) {
        die('Please refresh window and check it again');
    }
    $settings = isset($_REQUEST['settings']) ? filter_var_array($_REQUEST['settings'], FILTER_SANITIZE_STRING) : null;
    if ($settings !== null) {
        // Layout Settings
        // $settings = filter_var_array($_REQUEST['settings'],FILTER_SANITIZE_STRING);
