 ...
 * Transfer Items
 *
 * @package     GamiPress\Transfers\Custom_Tables\Transfer_Items
 * @since       1.0.0
 */
...
/**
 * Parse query args for transfer items
 *
 * @since  1.0.0
 *
 * @param string $where
 ...
 * @return string
 */
function gamipress_transfers_transfer_items_query_where( $where, $ct_query ) {

    global $ct_table;

    if( $ct_table->name !== 'gamipress_transfer_items' ) {
        return $where;
    }
...
add_filter( 'ct_query_where', 'gamipress_transfers_transfer_items_query_where', 10, 2 );