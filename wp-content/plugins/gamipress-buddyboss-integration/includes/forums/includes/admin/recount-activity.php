...
function gamipress_bbp_recountable_activity_triggers( $recountable_activity_triggers ) {
    ...
    $recountable_activity_triggers[__( 'BuddyBoss Forums', 'gamipress-buddyboss-integration' )] = array(
        'bbp_activities' => __( 'Recount forum activities (forums, topics and replies)', 'gamipress-buddyboss-integration' ),
        'bbp_favorites' => __( 'Recount topics favorites', 'gamipress-buddyboss-integration' ),
    );
...
function gamipress_bbp_activity_recount_activities( $response, $loop, $limit, $offset ) {
    ...
    if( $loop === 0 && $posts_count > $limit ) {
        ...
        $response['message'] = sprintf( __( '%d items found, recounting...', 'gamipress-buddyboss-integration' ), $posts_count );
        $response['log'] = sprintf( __( '%d items found, recounting...', 'gamipress-buddyboss-integration' ), $posts_count ) . "<br>\n";
    ...
    if( $recounted_posts <= $posts_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining items to finish recount', 'gamipress-buddyboss-integration' ), ( $posts_count - $recounted_posts ) );
    }
...
function gamipress_bbp_activity_recount_favorites( $response, $loop, $limit, $offset ) {
    ...
    if( $loop === 0 && $users_count > $limit ) {
        ...
        $response['log'] = sprintf( __( '%d users found, recounting...', 'gamipress-buddyboss-integration' ), $users_count ) . "<br>\n";
