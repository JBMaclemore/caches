...
function gamipress_bp_recountable_activity_triggers( $recountable_activity_triggers ) {
    ...
    // BuddyBoss
    $recountable_activity_triggers[__( 'BuddyBoss', 'gamipress-buddyboss-integration' )] = array(
        'bp_activated_users' => __( 'Recount activated accounts', 'gamipress-buddyboss-integration' ),
    );

    // BuddyBoss Friendships
    if ( gamipress_bp_is_active( 'friends' ) ) {
        $recountable_activity_triggers[__( 'BuddyBoss Friendships', 'gamipress-buddyboss-integration' )] = array(
            'bp_friendships'   => __( 'Recount friendships', 'gamipress-buddyboss-integration' ),
        );
    }

    // BuddyBoss Messages
    if ( gamipress_bp_is_active( 'messages' ) ) {
        $recountable_activity_triggers[__( 'BuddyBoss Messages', 'gamipress-buddyboss-integration' )] = array(
            'bp_messages' => __( 'Recount messages sent/replied', 'gamipress-buddyboss-integration' ),
        );
    }

    // BuddyBoss Activity
    if ( gamipress_bp_is_active( 'activity' ) ) {
        $recountable_activity_triggers[__( 'BuddyBoss Activity', 'gamipress-buddyboss-integration' )] = array(
            'bp_published_activities'       => __( 'Recount activity stream messages', 'gamipress-buddyboss-integration' ),
            'bp_activity_comments'          => __( 'Recount activity replies', 'gamipress-buddyboss-integration' ),
            'bp_activity_favorites'         => __( 'Recount activity favorites', 'gamipress-buddyboss-integration' ),
        );
    }

    // BuddyPress Groups
    if ( gamipress_bp_is_active( 'groups' ) ) {
        $recountable_activity_triggers[__( 'BuddyBoss Groups', 'gamipress-buddyboss-integration' )] = array(
            'bp_group_published_activities'   => __( 'Recount groups activity stream messages', 'gamipress-buddyboss-integration' ),
            'bp_group_joins'                  => __( 'Recount groups joins', 'gamipress-buddyboss-integration' ),
            'bp_group_promotions'             => __( 'Recount groups promotions to moderator/administrator', 'gamipress-buddyboss-integration' ),
        );
    }
...
function gamipress_bp_activity_recount_activated_users( $response, $loop ) {
    ...
    if( $loop === 0 && $users_count > $limit ) {
        ...
        $response['message'] = sprintf( __( '%d users found, recounting...', 'gamipress-buddyboss-integration' ), $users_count );
    ...
    if( $recounted_users < $users_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining users to finish recount', 'gamipress-buddyboss-integration' ), ( $users_count - $recounted_users ) );
    }
...
function gamipress_bp_activity_recount_friendships( $response, $loop ) {
    ...
    if ( ! gamipress_bp_is_active( 'friends' ) ) {
        ...
        $response['response'] = __( 'BuddyBoss Friends module is not active.', 'gamipress-buddyboss-integration' );
    ...
    if( $loop === 0 && $friendships_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d friendships found, recounting...', 'gamipress-buddyboss-integration' ), $friendships_count );
    ...
    if( $recounted_friendships < $friendships_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining friendships to finish recount', 'gamipress-buddyboss-integration' ), ( $friendships_count - $recounted_friendships ) );
    }
...
function gamipress_bp_activity_recount_messages( $response, $loop ) {
    ...
    if ( ! gamipress_bp_is_active( 'messages' ) ) {
        $response['success'] = false;
        $response['response'] = __( 'BuddyBoss Messages module is not active.', 'gamipress-buddyboss-integration' );
    ...
    if( $loop === 0 && $messages_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d messages found, recounting...', 'gamipress-buddyboss-integration' ), $messages_count );
    ...
    if( $recounted_messages < $messages_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining messages to finish recount', 'gamipress-buddyboss-integration' ), ( $messages_count - $recounted_messages ) );
    }
...
function gamipress_bp_activity_recount_activities( $response, $loop ) {
    ...
    if ( ! gamipress_bp_is_active( 'activity' ) ) {
        $response['success'] = false;
        $response['response'] = __( 'BuddyBoss Activity module is not active.', 'gamipress-buddyboss-integration' );
    ...
    if( $loop === 0 && $activities_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d activities found, recounting...', 'gamipress-buddyboss-integration' ), $activities_count );
    ...
    if( $recounted_activities < $activities_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining activities to finish recount', 'gamipress-buddyboss-integration' ), ( $activities_count - $recounted_activities ) );
    }
...
function gamipress_bp_activity_recount_activity_group_published_activities( $response, $loop ) {
    ...
    if ( ! gamipress_bp_is_active( 'groups' ) ) {
        $response['success'] = false;
        $response['response'] = __( 'BuddyBoss Groups module is not active.', 'gamipress-buddyboss-integration' );
    ...
    if( $loop === 0 && $groups_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d groups found, recounting...', 'gamipress-buddyboss-integration' ), $groups_count );
    ...
    if( $recounted_groups < $groups_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining groups to finish recount', 'gamipress-buddyboss-integration' ), ( $groups_count - $recounted_groups ) );
    }
...
function gamipress_bp_activity_recount_activity_group_promotions( $response, $loop ) {
    ...
    if( $loop === 0 && $groups_members_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d group members found, recounting...', 'gamipress-buddyboss-integration' ), $groups_members_count );
    ...
    if( $recounted_group_members < $groups_members_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining group members to finish recount', 'gamipress-buddyboss-integration' ), ( $groups_members_count - $recounted_group_members ) );
    }
