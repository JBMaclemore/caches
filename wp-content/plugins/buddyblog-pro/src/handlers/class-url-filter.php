...
class URL_Filter {
	...
	private function setup() {
		...
		add_filter( 'attachment_link', array( $this, 'attachment_permalink_filter' ), 10, 2 );
	}
	...
	public function post_edit_link_filter( $url, $post_id, $context ) {
		...
		if ( ! bblpro_user_can_edit_post( get_current_user_id(), $post->ID ) ) {
			return $url;
		}
	...
	public function attachment_permalink_filter( $url, $attachment_id ) {
		...
		// check if it is BuddyBlog Post attachment
		//if ( ! get_post_meta( $attachment->post_parent, '_is_buddyblog_post', true ) ) {
			// return $url;
		//}
		...
		// it is viewable on profile.
		return trailingslashit( get_permalink( $post ) ) . 'attachment/' . $attachment_id . '/';
		// It is BuddyBlog post,
		//return home_url( user_trailingslashit( $attachment->post_name ) );
	}
