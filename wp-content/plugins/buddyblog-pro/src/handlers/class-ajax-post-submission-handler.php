...
class Ajax_Post_Submission_Handler {
	...
	public function save_post() {
		...
		$pre_validate = apply_filters( 'bblpro_pre_validate_form_submission', true, $form, $_POST );

		// if the pre validation fails, return.
		if ( ! $pre_validate ) {

			if ( is_wp_error( $pre_validate ) ) {
				wp_send_json_error( array(
					'global' => $pre_validate->get_error_message(),
				) );
			} else {
				wp_send_json_error( $error_messages['invalid_action'] );
			}
		...
		if ( $post_status === $default_status && $old_status !== $default_status ) {
			...
			$is_submission = false; // it is update not submission.
		}

		// allows extending post data if needed.
		$post_data = (array) apply_filters( 'bblpro_post_submission_prepared_data', $post_data, $form_id, $_POST, $is_submission );
		// validate
		...
		if ( $post_status === $default_status && $old_status !== $default_status ) {
			...
			$redirect_url = bblpro_form_get_submission_redirect_url( $form_id, $post_id );
			...
			$redirect_url = bblpro_user_can_edit_post( get_current_user_id(), $post_id ) ? bblpro_get_post_edit_url( $post_id ) : get_permalink( $post_id );
			$redirect_url = add_query_arg( array( 'updated' => 1 ), $redirect_url );
