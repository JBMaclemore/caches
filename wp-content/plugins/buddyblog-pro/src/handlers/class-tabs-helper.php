...
class Tabs_Helper {
	...
	private function add_tabs( $user_id, $post_type, $args ) {
		...
		// Action handlers.
		// Is editing enabled? or post creation enabled, we allow edit url in both the cases.
		// @todo re-check the access for edit.
		if ( bblpro_is_post_editing_enabled( $logged_id, $post_type ) || ( bblpro_is_action_enabled( $user_id, $post_type, 'create' ) && bblpro_is_action_visible( $user_id, $post_type, 'create', $visitor_id ) ) ) {
			add_action(
				'bp_actions',
			...
			add_action(
				...
				function () use ( $post_type ) {
					...
					if ( bp_is_current_component( bblpro_get_tab_slug( $post_type ) ) && bp_is_current_action( $delete_slug ) ) {
						View_Helper::instance()->process( $post_type, 'delete', bp_action_variable( 0 ) );
					}
				}
			);
		}
	...
	private function add_post_type_tab( $user_id, $post_type, $args ) {
		...
		if ( bp_is_user() ) {
			...
			$post_status = apply_filters( 'bblpro_post_type_tab_count_included_statuses', $post_status, $post_type, $user_id );
