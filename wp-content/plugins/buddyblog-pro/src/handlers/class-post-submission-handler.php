...
class Post_Submission_Handler {
	...
	public function handle() {
		...
		if ( ! $pre_validate ) {
			...
			if ( is_wp_error( $pre_validate ) && method_exists( buddyblog_pro()->errors, 'merge_from' ) ) {
				buddyblog_pro()->errors->merge_from( $pre_validate );
			}
		...
		*/
		...
		if ( $post_status === $default_status && $old_status !== $default_status ) {
			...
			$redirect = bblpro_form_get_submission_redirect_url( $form_id, $post_id );
		} else {
