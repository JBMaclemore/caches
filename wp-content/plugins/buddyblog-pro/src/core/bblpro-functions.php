...
function bblpro_get_posts_query_args( $args ) {
	...
	if ( bblpro_get_option( $args['post_type'] . '_show_front_posts_only', 0 ) ) {
		// If BudyBlog Groups is installed, filter blog posts by user only.
...
function bblpro_paginate() {
	...
	if ( $total > 1 ) {
		...
		$base_url = bblpro_get_post_type_tab_url( bp_displayed_user_id(), bblpro_get_current_post_type() );

		$base_url = trailingslashit( apply_filters( 'bblpro_pagination_base_url', $base_url ) ) . bblpro_get_current_action();
 ...
 * @return WP_Post|null
 */
function bblpro_get_current_deletable_post() {

	$post = null;
	if ( 'delete' === bblpro_get_current_action() ) {
 ...
 * @return int
 */
function bblpro_get_current_deletable_post_id() {

	$post = bblpro_get_current_deletable_post();
