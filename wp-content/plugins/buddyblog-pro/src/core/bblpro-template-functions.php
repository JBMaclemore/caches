...
function bblpro_get_template_part( $post_type, $slug, $name = '', $fallback_path = '' ) {
	...
	$template = bblpro_locate_post_type_template( $post_type, $templates, false, $fallback_path );
 ...
 *
 * @deprecated since 1.2.6, Use bblpro_locate_post_type_template() instead.
 *
...
function bblpro_locate_template( $post_type, $template_files, $load = false, $default_path = '' ) {
	return bblpro_locate_post_type_template( $post_type, $template_files, $load, $default_path );
}

/**
 * Locates a template for give post type and returns the path for inclusion.
 *
 * This is the load order:
 *
 *        your-child-theme  /    buddyblog    / default(current template pack) / post_type|default /   $template_name
 ...
 * @return string
 */
function bblpro_locate_post_type_template( $post_type, $template_files, $load = false, $default_path = '' ) {
...
/**
 * Locate a template file and return the path for inclusion.
 *
 * This is the load order:
 *
 *        your-child-theme  /    buddyblog    / default(current template pack) /  $template_name
 *        your-theme        /    buddyblog    / default(current template pack) /   $template_name
 *        wp-content/plugins/buddyblog-pro/templates/        /    buddyblog    / default(current template pack) /   $template_name
 *
 *        $default_path     / default(current template pack) /   $template_name
 *
 * @see bblpro_get_template_part() if you are looking to load template parts.
 *      This function is aimed at plugin developer.
 *
 * @param array|string  $template_files array of php templates.
 * @param bool          $load whether to load or return the path.
 * @param string        $default_path (default: ''), path to use as base. Allows plugins to override it.
 *
 * @return string
 */
function bblpro_locate_template_file( $template_files, $load = false, $default_path = '' ) {
	...
	foreach ( $template_stacks as $template_stack => $load_order ) {
		...
		foreach ( $template_files as $template_file ) {
			...
			// locate theme/buddyblog/template-pack-id/file.
			$theme_template_file = $base_dir . '/' . $template_stack . '/'. $template_file;

			$located = locate_template( array( $theme_template_file ), false );
			// fallback to plugin/templates/buddyblog/template-pack-id/post_type/files.
			if ( ! $located && is_readable( $default_path . '/' . $template_stack . '/' . $template_file ) ) {
				$located = $default_path . '/' . $template_stack . '/' . $template_file;
			}
	...
	// Return what we found.
	return apply_filters( 'bblpro_located_template_file', $located, $template_files, $default_path );
}
