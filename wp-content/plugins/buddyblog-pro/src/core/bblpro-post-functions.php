...
function bblpro_get_post_edit_url( $post_id ) {
	...
	return apply_filters( 'bblpro_edit_post_url', $url, $post_id );
}
...
function bblpro_get_post_delete_url( $post_id, $screen = '' ) {
	...
	return apply_filters( 'bblpro_delete_post_url', add_query_arg( array( 'screen' => $screen ), $url ), $post_id, $screen );
}
 ...
 */
function bblpro_get_default_post_to_edit( $post_type = 'post', $create_in_db = false, $form_id = 0 ) {

	$post = apply_filters( 'bblpro_default_post_to_edit', null, $post_type, $create_in_db, $form_id );
	...
	if ( $create_in_db ) {
		...
		if ( $form_id ) {
			$default_post_args['comment_status'] = bblpro_form_get_default_comment_status( $form_id );
		}

		$post_id = wp_insert_post( $default_post_args );
		...
		$post->to_ping        = '';
		$post->pinged         = '';
		$post->comment_status = $form_id ? bblpro_form_get_default_comment_status( $form_id ) : get_default_comment_status( $post_type );
 ...
 */
function bblpro_get_user_posts_count( $user_id, $post_type = '', $statuses = 'publish' ) {
	...
	if ( empty( $statuses ) || in_array( 'any', $statuses, true ) ) {
		...
		$where[] = sprintf( 'post_status IN (%s)', join( ',', $statuses ) );
	}
...
/**
 * Prints button classes.
 *
 * @param string $type button type.
 * @param string $btn_classes extra button classes.
 */
function bblpro_button_classes( $type, $btn_classes = '' ) {
	echo esc_attr( join( ' ', bblpro_get_button_classes( $type, $btn_classes ) ) );
}

/**
 * Returns an array of sanitized button classes.
 *
 * @param string $type button type.
 * @param string $btn_classes extra button classes.
 *
 * @return array
 */
function bblpro_get_button_classes( $type, $btn_classes = '' ) {
	$classes = array();
	...
	return (array) apply_filters( 'bblpro_get_button_classes', $classes, $type, $btn_classes );
}

/**
 * Prints classes for post submission button.
 *
 * @param string       $context button context('post-create' or 'post-edit' screen).
 * @param string       $btn_classes button classes.
 * @param string       $post_type post type.
 * @param WP_Post|null $post post object or null.
 */
function bblpro_submit_button_classes( $context, $btn_classes = '', $post_type = '', $post = null ) {
	echo esc_attr( join( ' ', bblpro_get_submit_button_classes( $context, $btn_classes, $post_type, $post ) ) );
}

/**
 * Returns an array of classes for post submission button.
 *
 * @param string       $context button context('post-create' or 'post-edit' screen).
 * @param string       $btn_classes button classes.
 * @param string       $post_type post type.
 ...
 * @return array
 */
function bblpro_get_submit_button_classes( $context, $btn_classes = '', $post_type = '', $post = null ) {
	return (array) apply_filters( 'bblpro_submission_button_classes', bblpro_get_button_classes( 'post-submission', $btn_classes ), $context, $post_type, $post );
}
 ...
 * @param int| WP_Post  $form_id form id or object.
 *
 * @return mixed|void
 */
function bblpro_get_submit_button_label( $post_type, $post = null, $form_id = null ) {
...
function bblpro_parse_tokens( $text, $post ) {
	...
	$tokens = (array) apply_filters( 'bblpro_message_tokens', $tokens, $post, $user, $text );
